...
环境变量通常用于存储一些与系统操作相关的命令, 比如如JAVA\_HOME. 在脚本中设置的环境变量, 只在当前脚本中生效, 不会修改系统的环境变量. 通常用于为某些程序的运行准备环境. 比如说, 系统默认安装的是jdk1.7, 而你的程序想要使用jdk 1.8运行 ,那么就可以创建一个脚本, 脚本中先设置JAVA\_HOme 的值, 然后再执行你的程序就好了.
...
** 输出结果: **  
脚本中将环境变量JAVA\_HOME 删除了, 脚本中便获取不到JAVA\_HOME 环境变量了, 但是脚本执行完之后, 直接输出环境变量JAVA\_HOME, 会发现依然存在.
