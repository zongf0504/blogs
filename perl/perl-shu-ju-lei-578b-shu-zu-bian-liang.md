...
| 数组上下文 | 示例 | 描述 |
| :--- | :--- | :--- |
| 数组上下文 | @array_1 = @array_name; | 等号左侧为数组, 因此此语句表示复制数组 |
| 标量上下文 | $length = @array_name | 等号左侧为标量, 标量表示单数, 所以此表达式为获取数组长度 |
| 哈希上下文 | %hash = @array_name | 等号左侧为哈希变量, 则表示将数组转换为哈希结构 |
...
* 使用splice 可以进行删除替换数组元素, 语法: splice(目标数组， 开始数组下标， 删除个数， 替换数组)
